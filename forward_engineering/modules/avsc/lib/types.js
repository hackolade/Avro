	...
	if (numWrappedUnions) {
		...
		try {
			unionType = Type.forSchema(
				Object.keys(branchTypes).map(function (name) {
...
function RecordType(schema, opts) {
	...
	if (
		utils.hasDuplicates(schema.fields, function (f) {
			return f.name;
		})
	) {
...
RecordType.prototype._update = function (resolver, type, opts) {
	...
	for (i = 0; i < rFields.length; i++) {
		...
		if (matches.length > 1) {
			Type.addError(new Error())(f('ambiguous aliasing for %s.%s (%s)', type.name, field.name, matches));
		}
		if (!matches.length) {
			if (field.defaultValue() === undefined) {
				Type.addError(new Error())(f('no matching field for default-less %s.%s', type.name, field.name));
			}
